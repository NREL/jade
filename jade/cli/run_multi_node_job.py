...
def run_multi_node_job(jade_runtime_output, verbose, manager_script_and_args):
    ...
    if result == "true":
        ret = run_manager(jade_runtime_output, verbose, manager_script_and_args)
    else:
        ...
        ret = run_worker(jade_runtime_output, verbose)

    return ret


def run_manager(output_dir, verbose, manager_script_and_args):
    """Run the manager instance."""
    shutdown_file = _get_shutdown_file(output_dir)
    if shutdown_file.exists():
        os.remove(shutdown_file)
    try:
        return _run_manager(output_dir, verbose, manager_script_and_args)
    finally:
        # Notify the workers to shutdown.
        shutdown_file.touch()


def _run_manager(output_dir, verbose, manager_script_and_args):
    filename = os.path.join(output_dir, "run_multi_node_job_manager.log")
...
def run_worker(output_dir, verbose, poll_interval=60):
    """Run a worker instance."""
    hostname = socket.gethostname()
    filename = os.path.join(output_dir, f"run_multi_node_job_worker_{hostname}.log")
    level = logging.DEBUG if verbose else logging.INFO
    setup_logging(__name__, filename, file_level=level, console_level=level, mode="w")
...
def _get_shutdown_file(output_dir):
    return Path(output_dir) / SHUTDOWN_FILENAME
